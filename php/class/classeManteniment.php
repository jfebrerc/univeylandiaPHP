...
class Manteniment{
  ...
  function __construct(){
   ...
   if (method_exists($this,$f)){
    ...
    public function __construct4( $idAtraccio,$idUsuari,  $dataInici,  $dataFi){
    ...
    public function RegistrarAssignacio(){
        ...
        $sql = "INSERT INTO MANTENIMENT_ATRACCIO (id_atraccio,id_usuari,data_inici,data_fi) VALUES (?,?,?,?);";
          $sentencia = $connection->prepare($sql);
          $sentencia->bind_param("iiss",$this->idAtraccio,$this->idUsuari,$this->dataInici,$this->dataFi);
  ...
  if ($result) {
      while($row = $result->fetch_assoc()) {
        $id_manteniment = $row["id_manteniment_atraccio"];
        $nom_empleat = $row["nom"];
        ...
        echo '      <td>'.$data_inici.'</td>';
        echo '      <td>'.$data_fi.'</td>';
        echo '      <td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalCenter'.$id_manteniment.'"> Modificar
                    </button></td>';
        echo '      <td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalEliminar'.$id_manteniment.'"> Eliminar
                    </button></td>';
        echo '    </tr>';
        echo '  </tbody>';
        ...
        echo '<!-- ELIMINAR -->
        <div class="modal fade" id="ModalEliminar'.$id_manteniment.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Atenció!</h5>
              ...
              <div class="modal-body">
                <div class="container">
                <form method="post">
                <input class="form-control" type="text" value="'.$id_manteniment.'" id="example-text-input" name="id_assignacioelim" style="display: none;">
        ...
        echo '<!-- MODIFICAR -->
        <div class="modal fade" id="exampleModalCenter'.$id_manteniment.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header">
              ...
              <div class="modal-body">
                <div class="container">
                  ...
                  <div class="form-group row">
                  <div class="col-10">
                    <input class="form-control" type="text" value="'.$id_manteniment.'" id="example-text-input" name="id_assignaciomod" style="display: none;">
                  </div>
                  </div>
                  <div class="form-group row">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="form-group row">
                        ...
                        <div class="col-10">
                        <input type="date" value="'.$data_inici.'" class="form-control form-control-sm" name="data_inici_assign">
                        </div>
                      </div>
                      <div class="form-group row">
                        <label for="example-text-input" class="col-2 col-form-label">Data fi: </label>
                        <div class="col-10">
                        <input type="date" value="'.$data_fi.'" class="form-control form-control-sm" name="data_fi_assign">
                        </div>
                      </div>
                    <div class="form-group row">
                      <div class="offset-sm-2 col-sm-10">
...
public static function modificarAssignacio(){
  $conexio = crearConnexio();
  $id_manteniment = $_POST['id_assignaciomod'];
  ...
  $sql_update = "UPDATE MANTENIMENT_ATRACCIO SET  id_atraccio='$atraccio',id_usuari='$empleat', data_inici='$data_inici', data_fi='$data_fi' WHERE id_manteniment_atraccio=$id_manteniment";
    if (mysqli_query($conexio, $sql_update)) {
        echo '<script>window.location.href = window.location.href + "?positivet";</script>';
        echo "<p> oket </p>";
    } else {
...
public static function eliminarAssignacio(){
    $conexio = crearConnexio();
    $id_mantenimentE = $_POST['id_assignacioelim'];
    $sql_eliminar ="DELETE FROM MANTENIMENT_ATRACCIO WHERE id_manteniment_atraccio=$id_mantenimentE";
      if (mysqli_query($conexio, $sql_eliminar)) {
        echo '<script>window.location.href = window.location.href + "?negativet";</script>';
        echo "Record updated successfully";
      }else {
...
/*  public static function exportarAssignacions(){
  ...
  if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        ...
        $columna_data_inici = $columna_data_inici.$data_inici."\n";
        $columna_data_fi = $columna_data_fi.$data_fi."\n";
      }
  } else {
  ...
  $pdf->SetY($Y_Table_Position);
  $pdf->SetX(160);
  $pdf->MultiCell(50,6,$columna_data_inici,1,'R');
  $pdf->SetY($Y_Table_Position);
  $pdf->SetX(210);
  $pdf->MultiCell(50,6,$columna_data_fi,1,'R');
