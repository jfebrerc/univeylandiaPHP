...
class Neteja{
  ...
  private $idUsuari;
  private $idAtraccio;
  private $data_inici_neteja;
  private $data_fi_neteja;
  ...
  function __construct(){
   ...
   if (method_exists($this,$f)){
    ...
    public function __construct4($idUsuari, $idAtraccio,  $data_inici_neteja,  $data_fi_neteja){
    $this->idUsuari=$idUsuari;
    $this->idAtraccio=$idAtraccio;
    $this->data_inici_neteja=$data_inici_neteja;
    $this->data_fi_neteja=$data_fi_neteja;
    ...
    public function RegistrarAssignacio(){
        ...
        $sql = "INSERT INTO NETEJA_ATRACCIO (id_usuari,id_atraccio,data_inici_neteja,data_fi_neteja) VALUES (?,?,?,?);";
          $sentencia = $connection->prepare($sql);
          $sentencia->bind_param("iiss",$this->idUsuari,$this->idAtraccio,$this->data_inici_neteja,$this->data_fi_neteja);
  ...
  if ($result) {
      while($row = $result->fetch_assoc()) {
        $id_neteja_atraccio = $row["id_neteja_atraccio"];
        $nom_empleat = $row["nom"];
        $cognom1_empleat = $row["cognom1"];
        $cognom2_empleat = $row["cognom2"];
        $dniEmpleat = $row["numero_document"];
        $nom_atraccio = $row["nom_atraccio"];
        $data_inici_neteja = $row["data_inici_neteja"];
        $data_fi_neteja = $row["data_fi_neteja"];
        $data_creacio_registre = $row["data_creacio_registre"];
        $id_atraccio = $row["id_atraccio"];
        ...
        echo '      <td>'.$data_inici_neteja.'</td>';
        echo '      <td>'.$data_fi_neteja.'</td>';
        echo '      <td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalCenter'.$id_neteja_atraccio.'"> Modificar
                    </button></td>';
        echo '      <td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalEliminar'.$id_neteja_atraccio.'"> Eliminar
                    </button></td>';
        echo '    </tr>';
        echo '  </tbody>';
        ...
        echo '<!-- ELIMINAR -->
        <div class="modal fade" id="ModalEliminar'.$id_neteja_atraccio.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Atenció!</h5>
              ...
              <div class="modal-body">
                <div class="container">
                <form method="post">
                <input class="form-control" type="text" value="'.$id_neteja_atraccio.'" id="example-text-input" name="id_neteja_atraccioelim" style="display: none;">
        ...
        echo '<!-- MODIFICAR -->
        <div class="modal fade" id="exampleModalCenter'.$id_neteja_atraccio.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
              <div class="modal-header">
              ...
              <div class="modal-body">
                <div class="container">
                  ...
                  <div class="form-group row">
                  <div class="col-10">
                    <input class="form-control" type="text" value="'.$id_neteja_atraccio.'" id="example-text-input" name="id_neteja_atracciomod" style="display: none;">
                  </div>
                  </div>
                  <div class="form-group row">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="form-group row">
                        ...
                        <div class="col-10">
                        <input type="date" value="'.$data_inici_neteja.'" class="form-control form-control-sm" name="data_inici_neteja">
                        </div>
                      </div>
                      <div class="form-group row">
                        <label for="example-text-input" class="col-2 col-form-label">Data fi: </label>
                        <div class="col-10">
                        <input type="date" value="'.$data_fi_neteja.'" class="form-control form-control-sm" name="data_fi_neteja">
                        </div>
                      </div>
                    <div class="form-group row">
                      <div class="offset-sm-2 col-sm-10">
...
public static function modificarAssignacio(){
  $conexio = crearConnexio();
  $id_neteja_atraccio = $_POST['id_neteja_atracciomod'];
  $atraccio = $_POST['atraccio_seleccionada'];
  $empleat = $_POST['empleat_seleccionat'];  //Extrema, mitjana, familiar, aquatica
  $data_inici_neteja = $_POST['data_inici_neteja'];
  $data_fi_neteja = $_POST['data_fi_neteja'];

  $sql_update = "UPDATE NETEJA_ATRACCIO SET id_usuari='$empleat', id_atraccio='$atraccio', data_inici_neteja='$data_inici_neteja', data_fi_neteja='$data_fi_neteja' WHERE id_neteja_atraccio=$id_neteja_atraccio";
    if (mysqli_query($conexio, $sql_update)) {
        echo '<script>window.location.href = window.location.href + "?positivet";</script>';
        echo "<p> oket </p>";
    } else {
...
public static function eliminarAssignacio(){
    $conexio = crearConnexio();
    $id_neteja_atraccioE = $_POST['id_neteja_atraccioelim'];
    $sql_eliminar ="DELETE FROM NETEJA_ATRACCIO WHERE id_neteja_atraccio=$id_neteja_atraccioE";
      if (mysqli_query($conexio, $sql_eliminar)) {
        echo '<script>window.location.href = window.location.href + "?negativet";</script>';
        echo "Record updated successfully";
      }else {
...
/*  public static function exportarAssignacions(){
  ...
  $columna_data_inici_neteja = "";
  $columna_data_fi_neteja = "";
  if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        ...
        $columna_data_inici_neteja = $columna_data_inici_neteja.$data_inici_neteja."\n";
        $columna_data_fi_neteja = $columna_data_fi_neteja.$data_fi_neteja."\n";
      }
  } else {
  ...
  $pdf->SetY($Y_Table_Position);
  $pdf->SetX(160);
  $pdf->MultiCell(50,6,$columna_data_inici_neteja,1,'R');
  $pdf->SetY($Y_Table_Position);
  $pdf->SetX(210);
  $pdf->MultiCell(50,6,$columna_data_fi_neteja,1,'R');
